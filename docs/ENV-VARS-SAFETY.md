...
| ~~Temporary Sentry test route (S0.1)~~ — retired at S0.2 (2026-09-25): route removed; the owner reports both Vercel entries deleted | ~~`SENTRY_TEST_TOKEN`~~ | — | — |
| Supabase project refs — the trusted identity guard of the privileged client, the health route, the proof route and the fixture tools (S0.2) | `SUPABASE_TEST_PROJECT_REF` / `SUPABASE_PROD_PROJECT_REF` | Private configuration, **not secret** (identifiers; no public prefix) | Both names in every Vercel scope and in `.env.local`; never derived from a URL or a key |
| Preview-only proof route `POST /api/setup-proof` (S0.2 → removed and revoked by S1.1) | `S0_2_PROOF_TOKEN` | Private (server-only) | Preview scope only and the owner's trusted proof process; at least 32 random characters; never Production or Development |
| Launch Gate and morning-check harness; Preview protection bypass | `PLAYWRIGHT_BASE_URL` / `VERCEL_AUTOMATION_BYPASS_SECRET` | Private (CI / server-only) | GitHub Actions secrets, the local shell or the owner's git-ignored `.env.local` only (every harness command loads that file, the shell wins); the bypass secret goes only to the exact validated Preview origin (S0.2), never into a URL, log, trace or report |
| Harness inputs (S0.2) | `PLAYWRIGHT_CANDIDATE_SHA` / `PLAYWRIGHT_TARGET_MODE` | Not secret | GitHub Actions variables / local shell only |
| Harness — deployment-evidence read (S0.2; optional) | `GITHUB_TOKEN` | Private (read-only; optional) | Local shell or GitHub Actions' own token only; only raises the GitHub API rate limit for the public deployment record; never required, never printed |
