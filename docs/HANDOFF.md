...
| Hosting — Vercel / `aalishaan-studio-app` | The owner's own Vercel account (owner, 2026-09-23) — Hobby during development, Pro before launch (`PROJECT-STATUS.md` §8c D-03); project created at S0.1 | the Aalishaan Studio owner (already the account holder) | No transfer — owner-held from the start; billing already the owner's; any other member removed or reduced to the agreed role at handoff; Production env values (Sensitive) stay in place, `VERCEL_AUTOMATION_BYPASS_SECRET` rotated per §2 |
...
| Error tracking — Sentry (D-23, installed at S0.1) | The owner's own Sentry account (owner, 2026-09-23); project created at S0.1 | the Aalishaan Studio owner (already the account holder) | No transfer — owner-held; any other member removed at handoff; Production alert rule points at the owner; `SENTRY_AUTH_TOKEN` rotated per §2 |
