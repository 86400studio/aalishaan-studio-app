...
| Database/auth — Supabase `aalishaan-studio-test` and `aalishaan-studio-prod` (D-05) | The owner's own Supabase account (owner, 2026-09-23) — Free during development, Pro before S2.20 (`PROJECT-STATUS.md` §8c D-05); projects created by the owner at S0.2 (2026-09-25; refs recorded by name in `PROJECT-STATUS.md` §9; `0000_init` applied on TEST the same day and on PROD by the owner's hand before the S0.2 merge); the weekly Free-plan health/pause check is the owner's (`PROJECT-STATUS.md` §9) | the Aalishaan Studio owner (already the account holder) | No transfer — owner-held; any other member removed at handoff; `supabase-dev` MCP (TEST) and the read-only `supabase-prod-readonly` (D-40) disconnected; `SUPABASE_SECRET_KEY` rotated per §2 |
