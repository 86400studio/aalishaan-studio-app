...
- [x] Before the merge, the PR head is verified against the reviewed head (`docs/WORKFLOW.md` §7): unchanged, or moved only by records-only commits — the returned review record appended (the AGENTS.md exemption) or that verdict recorded in the trackers — that change no code, config, schema, lockfile, runtime behavior or runtime doc (AGENTS.md; `docs/WORKFLOW.md` §6), with CI green at the head being merged; any other change after the review needs a new independent review first. The owner then merges and runs the Production smoke test. (S0.1: reviewed head `9d9a0a8`; merged head `cc82ae9`, reached by `a8fb3f5`, which only appends the returned review record, and `cc82ae9`, which only records the approval in `PROJECT-STATUS.md`, `ROADMAP.md` and the sprint record — records only, no path outside `docs/`; `/close` verified this and the Code Check at `cc82ae9` (run 36016924009, success) and returned GO; the owner merged PR #3 on 2026-09-24 as `d416bfb` and ran the Production smoke — S0.1 record, "Closure".)
