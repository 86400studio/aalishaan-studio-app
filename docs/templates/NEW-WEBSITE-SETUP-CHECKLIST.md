...
- [ ] Copy `ENVIRONMENT-PARITY.md` to `docs/`; fill its infrastructure and environment matrix (§3–§4), mark unused services N/A, and complete applicable setup steps in §6. Record later feature proofs as pending until those features exist; do not invent PASS results to finish setup. (S0.2, 2026-09-25: §3–§4 carry the S0.2 names and the TEST/PROD split; the project rows are filled from the owner's projects, created 2026-09-25; of the §12 S0.2 proofs, P8, P8b and P11 PASS on TEST since 2026-09-25 (night), P1 is partial, and P2, P10 and P12 are taken at the candidate — none prefilled.)
...
- [x] If Supabase is selected, follow `docs/SUPABASE-VERCEL-SETUP.md`: isolated non-production and Production projects, public/publishable values only in browser code, RLS before user data. (S0.2, 2026-09-25: the browser client reads only the two public values, the privileged client is `server-only` behind an enforced import boundary, and `0000_init` enables RLS with no policy before any data lands; the two projects exist (owner, 2026-09-25) and `0000_init` is applied and verified on TEST the same night — ticked.)
- [x] If a coding agent will use **Supabase MCP**, follow `docs/SUPABASE-MCP-SAFETY.md`: connect non-production first; production MCP stays disconnected unless a read-only exception is explicitly approved and recorded. (S0.2, 2026-09-25: `.mcp.json` holds the credential-free `supabase-dev` (TEST) entry and, by owner decision D-40, the read-only `supabase-prod-readonly` entry under Profile B — verification only; both connected through the owner's OAuth with manual tool approval on, and the §7 guardrail checks ran the same night (the production refused-write probe followed the same night and was refused — S0.2 record, Deviation 18) — ticked.)
...
- [ ] Follow `docs/ENVIRONMENT-PARITY.md` §7 for the selected data/auth profile: use reviewed migrations for new projects; capture an existing production schema only when its baseline is missing. Verify isolation before any Preview writes; complete feature-specific proofs with the sprint that introduces that feature. (S0.2, 2026-09-25: the reviewed migration `0000_init` is the baseline for both new projects; isolation is verified before any Preview write by `pnpm db:test:preflight` and the proof route's own guards; P8/P8b/P11 ran on TEST 2026-09-25 (night) and the Preview-origin proofs run at the candidate.)
