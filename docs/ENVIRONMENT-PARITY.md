...
| Database/auth TEST and PROD | Created by the owner 2026-09-25 — `aalishaan-studio-test` = `kivaatbvxifunilxoxde` (Local/Preview) / `aalishaan-studio-prod` = `mttuqbpfdhzhnsjsmjcf` (Production), refs in `PROJECT-STATUS.md` §9; `0000_init` applied and verified on TEST 2026-09-25 (night), PROD still without it — the reviewed pending migration of §7.10, applied by the human owner before the S0.2 merge. The code is wired for the split: Preview, Development and a local process resolve to the TEST ref, Production to the PROD ref, and every privileged path fails closed on a mismatch or on equal refs (`src/lib/supabase/config.ts`); the baseline `0000_init` is applied on TEST and pending on PROD; Auth is not configured (staff auth is S1.2) |
...
- `P1 · PASS (round 1 at 2a44441abd19c1a546597ed25859e078c415cae7; re-proven at the round-2 candidate — handoff) · 2026-09-25 (night) · 2a44441abd19c1a546597ed25859e078c415cae7 · TEST kivaatbvxifunilxoxde · S0.2 record → "Checks run" / "Pending remote evidence" 3 · **table half PASS (night, candidate 2a44441):** \`pnpm test:preview-proof --apply\` — marker s0-2-proof:90d0ca08236a9945 created, read and cleaned up by the deployed Preview's own privileged path, found in TEST only from the trusted process, absent afterwards. **Public half on the deployed candidate (Codex rounds 1 and 2):** from the round-3 candidate on, \`GET /api/health\` answers \`ok\` only when this build's own public client (\`createBrowserSupabaseClient()\`, the build-inlined publishable key and URL) points at the project the privileged identity verified, is accepted by it, and is denied at the table (\`42501\`, or an explicit empty array when the privileged read saw a row — a schema-missing or malformed answer never counts) — asserted by the Preview smoke at every candidate. Round-2 evidence at bd60f5cdfba42d5e4bd48906a69dbffbae861468: health 200, smoke 12/12, proof marker s0-2-proof:3154e195747b4038 (before the round-2 tightening). **Provenance half PASS 2026-09-25 (evening):** the owner created both projects and entered the values (PROJECT-STATUS §9); \`pnpm db:test:preflight\` in the trusted process — secret key accepted (REST root, a secret-only endpoint), publishable key accepted (Auth health), URL ref = SUPABASE_TEST_PROJECT_REF ≠ SUPABASE_PROD_PROJECT_REF, baseline absent, anonymous read 404; statuses only, never a value. As designed, the read-only provenance half runs first, before the baseline exists: \`pnpm db:test:preflight\` probes the public and the privileged key separately against the intended project's API (an opaque key is never decoded), requires the URL's ref to equal SUPABASE_TEST_PROJECT_REF and to differ from SUPABASE_PROD_PROJECT_REF, and reports statuses only; the table-backed half (the deployed Preview's own privileged path writing and reading the marker) follows the authorised TEST apply — \`pnpm test:preview-proof\``
- `P2 (pattern) · PASS (round 1 at 2a44441abd19c1a546597ed25859e078c415cae7; repeated at the round-3 candidate — handoff) · 2026-09-25 (night) · 2a44441abd19c1a546597ed25859e078c415cae7 · TEST kivaatbvxifunilxoxde; PROD mttuqbpfdhzhnsjsmjcf read-only control · S0.2 record → "Pending remote evidence" 3 · round 1: marker s0-2-proof:90d0ca08236a9945 created from the Preview, read, found in TEST only, cleaned up, absent afterwards; PROD control to_regclass('public.system_checks') null before and after through supabase-prod-readonly (no table, so no marker can exist there). As designed: unblocked by the TEST apply of 2026-09-25 (night); the read-only PROD control already reads to_regclass('public.system_checks') → null through supabase-prod-readonly (no table, so no marker can exist there); after P1: \`pnpm test:preview-proof --apply\` against the verified candidate Preview creates one \`s0-2-proof:<id>\` marker from the deployed route, confirms it from the trusted local process's privileged read of TEST, prints the exact read-only PROD query for the owner (schema presence, then the marker count when the table exists — run by the owner in the PROD SQL editor or by the builder through the read-only `supabase-prod-readonly` connection — D-40, Profile B), then cleans up and confirms absence; a residual fails the proof and is recorded`
- `P8 · PASS (TEST half; the PROD comparison at Phase D) · 2026-09-25 (night) · c678523ab4decea424094c1bc9837a8902ee8838 (the tree at apply time; the migration file is unchanged since 185592f) · TEST kivaatbvxifunilxoxde, ledger 0000 | init · docs/database-changes/S0.2-0000-init.md → "Applied and verified" · the schema/access inventory read through supabase-dev matches the record exactly — columns, constraints, trigger, function properties (prosecdef false, search_path empty), grants (service_role with seven privileges: the four explicit plus REFERENCES/TRIGGER/TRUNCATE from the platform defaults; no other API role), relrowsecurity true and not forced, no policy, the ledger row; supabase db dump could not run (Docker engine not running — S0.2 record → Deviation 16); PROD read-only through supabase-prod-readonly: to_regclass('public.system_checks') → null and no migration — the expected difference is exactly 0000_init (§7.10); the PROD comparison follows the human apply`
- `P8b · PASS · 2026-09-25 (night) · c678523ab4decea424094c1bc9837a8902ee8838 · TEST kivaatbvxifunilxoxde · tests/integration/baseline.test.ts; S0.2 record → "Checks run" · pnpm test:integration in the trusted local process under the owner's integration: fixture authorisation, after the TEST apply: 1 file, 8 passed — privileged create/read/update/delete of a namespaced synthetic row with the updated_at trigger moving, constraint violations, anonymous read/insert/update/delete denied with privileged post-checks (anonymous read HTTP 401), a scoped reset sparing a same-namespace and an other-namespace neighbour, the foreign-namespace refusal; no residual row. The hermetic denied-state scaffold in pnpm test:unit (335 tests, 2026-09-25) is not P8b evidence`
- `P10 (Preview half) · PASS (round 1 at 2a44441abd19c1a546597ed25859e078c415cae7; repeated at the round-3 candidate — handoff) · 2026-09-25 (night) · 2a44441abd19c1a546597ed25859e078c415cae7 · GitHub deployment 6665282007 (Vercel Preview https://aalishaan-studio-hza41oy15-86400studios-projects.vercel.app, same SHA) · S0.2 record → "Pending remote evidence" 3 · \`pnpm test:e2e\` 12/12 on desktop and mobile-390: the holding page through the host-scoped bypass cookie, six headers + noindex, \`GET /api/health\` 200, the removed route 404, the proof route denied 404, 302 to Vercel's login without the bypass. Earlier the same day the smoke (\`pnpm test:e2e\`, desktop + mobile-390) passed 12/12 in local mode against \`next start\` on 2026-09-25 — not Preview evidence; the Preview run verifies the unbypassed 302 to Vercel's login and the app's response through the host-scoped bypass cookie`
- `P11 · PASS · 2026-09-25 (evening; rechecked at night before and after the TEST apply — afterwards system_checks present and the anonymous read 401) · 185592f5e72318a85678ffcf0fe2ab256ff95307, rechecked on c678523ab4decea424094c1bc9837a8902ee8838 · TEST kivaatbvxifunilxoxde · scripts/testing/preflight.mjs (\`pnpm db:test:preflight\` exit 0: Auth health 200 with the secret key and with the publishable key, REST root 200 with the secret key; rechecked before each external run) · before each external run: the Auth health probe and both key probes must answer 200, the secret-key read of system_checks must answer 200 or (pre-apply) 404, and the anonymous read must be positively denied — 401/403, a 404 before the apply, or an empty 200 with a visible row — or the preflight fails (Codex round 1, finding 1) (a paused Free project fails here, not inside a test); pause owner and cadence recorded in PROJECT-STATUS §9 (the owner, weekly, and before each run); the disabled morning workflow is not a keepalive`
- `P12 · PASS (owner-confirmed 2026-09-25, late; names only) · 2026-09-25 · 2a44441abd19c1a546597ed25859e078c415cae7 (round 1; the configuration is unchanged at the round-3 candidate) · Vercel project aalishaan-studio-app · owner screenshot of the complete 23-variable project list, values never shown · confirmed: both Supabase projects and refs, that the publishable and secret keys of each scope belong to that project (TEST in Preview/Development/local, PROD in Production), no branch-specific override, the two refs in every scope, S0_2_PROOF_TOKEN in Preview only, SENTRY_TEST_TOKEN removed everywhere (owner-reported 2026-09-25), the Upstash TEST database and Turnstile test keys in Preview/Development, and the Protection Bypass secret held in the trusted process. Setup reported done 2026-09-25 (dashboard screenshot of the variable list; Gate 0 Upstash/Turnstile test values present; all three secrets held in .env.local — the TEST SUPABASE_SECRET_KEY, S0_2_PROOF_TOKEN and VERCEL_AUTOMATION_BYPASS_SECRET — rotated by the owner after a screenshot of .env.local was exposed, owner-reported 2026-09-25, correcting the earlier "partial paste in chat" note; the rotated TEST secret key is the one the preflight accepts). Open at 22:20 IST (S0.2 record → "Pending remote evidence" 1): the Vercel rows for SUPABASE_SECRET_KEY and S0_2_PROOF_TOKEN still showed a last update before the exposure — the owner re-confirms the replacement before Phase D`
