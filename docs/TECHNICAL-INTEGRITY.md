...
- [x] **Claude Code (one normal PR at S0.1, or the S0.1a PR when S0.1 is split — decision D-25):** strict `tsconfig`, ESLint + Prettier configs that exclude `prototype/`, the scripts above with the first real unit test, `engines` + `.nvmrc`, and the workflow file. No product behavior rides along. *(2026-09-24: all of it exists on `claude/s0.1-setup-scaffold` and passes locally — the six commands green, 51 unit tests, an empty suite proven to fail; S0.1 runs as one PR. Opened as PR #3 on 2026-09-24 after the owner authorised commit and push; first Code Check run 36002643091 — success.)*
- [x] **You (2 minutes, at S0.1, after the Code Check has run once on that PR):** edit the same `main` ruleset → tick **"Require status checks to pass"** → add **"Code Check"** → save.
- [x] **You + Claude Code (verify once, on that S0.1 PR):** the "Code Check" shows as **Required**; push one more commit and confirm the merge button is blocked while the check runs and unlocks only when it goes ✅. An unverified gate is the same as no gate.
